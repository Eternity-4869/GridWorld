...
public class Solution extends Jigsaw {
    ...
    public boolean BFSearch(JigsawNode bNode, JigsawNode eNode) {
        ...
        //如果队列为空，则搜索失败，问题无解;否则循环直到求解成功
        while (!openList.isEmpty()) {
            mySearchedNodesNum++;

            // 取出exploreList的第一个节点N，置为当前节点为currentJNode
            ...
            //寻找所有与currentJNode邻接且未曾被发现的节点，将它们插入open队列中
        	...
        	for (int j = 1; j < dimension * dimension; j++) {
        		if (jNode.getNodesState()[i] != 0 
        		 ...
        		 manhattanDist += Math.abs(x1 - x2) + Math.abs(y1 - y2);
        			euclideanDist += Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
        			break;
        		}
        	}        
        }
    
        int weight = s * 6 + manhattanDist * 3 + euclideanDist * 1;
